...
観測誤差は二成分の混合正規（mixture normal）であり、、 基準分散 $R>0$ 、 スケール $C>1$ により以下のように表す。
$$
...
- $h$：観測方程式のAR項、$F$：状態遷移方程式のAR項、$R$：観測方程式のノイズ、 $C$：観測方程式のノイズ（状態$2$のスケーラー）、  $\tau$：状態遷移方程式のノイズの分散、$K_t\in\{0,1\}$：潜在 indicator 
...
**Backward smoothing**では、$x_{T-1}$のフィルタリング分布（$x_{T-1}|y_{T-1}$）を、$x_T|y_{T-1}$と$x_T$の差（すなわち$T-1$期時点の予測分布からの乖離＝$T$時点の情報を反映）に回帰することで、未来の情報を遡って取り込んでいく：
