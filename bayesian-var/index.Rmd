...
print("Omega_Mn")
as_tibble(Omega_Mn, rownames = " ") %>% gt() %>% fmt_number(decimals = 2)
```
...
V_Mn_unvec_sorted %>% 
  ...
  gt() %>% 
  fmt_number(decimals = 2) %>% 
  tab_header("V prior (diag. element)")
# print("diag(V)_vec")
# diag(V_Mn)[1:16] %>% round(4)
# diag(V_Mn)[17:32] %>% round(4)
# diag(V_Mn)[33:48] %>% round(4)
```
...
tibble(
  ...
  mutate(Coef = fct_relevel(Coef, 
                            str_subset(levels(Coef), "^lRGDP"),
                            str_subset(levels(Coef), "^lDEF"),
                            str_subset(levels(Coef), "^FF"))) %>% 
  arrange(Coef) %>% 
  gt(groupname_col = "Equation") %>% 
  fmt_number(decimals = 2) %>% 
  tab_header(title = "推計結果（点推定値；OLS v.s. Minnesota Prior）")
```
...
```{r REG soc dummy}
mu_socdum <- colMeans(df_var[(1:p) ,vars])
tau_socdum <- 1
Y_socdum <- rbind(Y, diag(mu_socdum)/tau_socdum)
X_socdum <- rbind(X, 
                  t(rbind(matrix(0, nrow = 1, ncol = M), kronecker(matrix(1, nrow = p, ncol = 1), diag(mu_socdum)/tau_socdum))))

A_socdum <- solve(t(X_socdum) %*% X_socdum) %*% t(X_socdum) %*% Y_socdum
rownames(A_socdum) <- coef_names_A  # X と同じ regressor 順序のはず
colnames(A_socdum) <- vars
```
...
res_soc  <- make_var_fit_forecast(df_var, vars = vars, p = p, A = A_socdum,  h = h)
```
...
df_ols_series <- mk_method_series(res_ols, "OLS", vars)
df_mn_series  <- mk_method_series(res_mn,  "Minnesota", vars)
df_soc_series <- mk_method_series(res_soc, "SOC", vars)
...
df_plot4 <- bind_rows(
  ...
  mutate(
    ...
    method = factor(method, levels = c("Actual", "OLS", "Minnesota", "SOC"))
  )
```
...
plots_3 <- lapply(levels(df_plot4$var), function(v) {
  dfi <- df_plot4 |> dplyr::filter(var == v)
...
#        This matches your A_OLS, A_Mn_post, A_socdum style.
#   p  : lag order
...
rho_soc <- var_spectral_radius(A_socdum, p = p, M = M, add_const = TRUE)
print(c(OLS = rho_ols, Minnesota = rho_mn, SOC = rho_soc))
```
...
```{r EXP fit in diff}
diff_vars <- intersect(c("lRGDP", "lDEF"), levels(df_plot4$var))

df_plot4_trans <- df_plot4 |>
...
# 3枚（変数ごと）プロット
plots_3_trans <- lapply(levels(df_plot4_trans$var), function(v) {
  dfi <- df_plot4_trans |> dplyr::filter(var == v)
...
```{r, eval = F}
# 1Q ahead MSFE (point forecast) for OLS / Minnesota / SOC
# using df_var_long (1980-01-01 - 2019-12-31)

# ---- prerequisites: you already have make_varmat_KK() defined ----
# make_varmat_KK(df, vars, p, add_const = TRUE)

# ---- helper: build x_t from last p actual observations ----
make_x_lastp <- function(Ymat, p, add_const = TRUE) {
  # Ymat: T x M (actual levels), last row is time t
  stopifnot(nrow(Ymat) >= p + 1)
  lag_block <- Ymat[(nrow(Ymat)):(nrow(Ymat) - p + 1), , drop = FALSE] # rows: t, t-1,... (latest first)
  # need rows = (t, t-1, ..., t-p+1) corresponds to (t, ..., t-p+1)
  # but regressor uses (t, ..., t-p+1) as lags for forecasting y_{t+1}
  x_no_const <- as.vector(t(lag_block))  # lag1(vars) -> lag2 -> ... (KK Xと一致)
  if (add_const) c(1, x_no_const) else x_no_const
}

# ---- estimate OLS A and residual covariance Omega_hat ----
estimate_ols_A_Omega <- function(df_train, vars, p, add_const = TRUE) {
  vm <- make_varmat_KK(df_train, vars = vars, p = p, add_const = add_const)
  Y <- vm$Y; X <- vm$X

  # OLS coefficient
  A_OLS <- solve(t(X) %*% X, t(X) %*% Y)  # K x M

  # residual covariance (unbiased-ish): U'U / (T_eff - K)
  U <- Y - X %*% A_OLS
  T_eff <- nrow(Y)
  K <- ncol(X)
  Omega_hat <- crossprod(U) / (T_eff - K)

  list(A = A_OLS, Omega = Omega_hat)
}

# ---- Minnesota posterior mean A (Sigma fixed at Omega_hat) ----
estimate_minnesota_A <- function(df_train, vars, p, a1 = 1, a2 = 0.5, a3 = 0.1, add_const = TRUE) {

  ols <- estimate_ols_A_Omega(df_train, vars = vars, p = p, add_const = add_const)
  Omega_Mn <- ols$Omega

  M <- length(vars)
  K <- (if (add_const) 1L else 0L) + M * p

  # --- build X, y in KK vec form (only needed for posterior mean)
  vm <- make_varmat_KK(df_train, vars = vars, p = p, add_const = add_const)
  X <- vm$X
  y <- vm$y

  # --- prior mean alpha_Mn (KM x 1)
  alpha_Mn <- matrix(0, M * K, 1)
  # positions of own first lag in each equation under your stacking:
  # equation i corresponds to block (i-1)*K + 1..K, and within block:
  # const at 1, lag1 vars are at 2..(M+1), so own-lag1 is at (1 + i)
  for (i in 1:M) {
    alpha_Mn[(i - 1) * K + (1 + i), 1] <- 1
  }

  # --- prior variance diag V_Mn (KM x KM)
  V_Mn_diag <- rep(NA_real_, M * K)
  V_Mn_r <- c(1, rep((1:p)^(-2), each = M)) # length K

  for (i in 1:M) {
    V_Mn_asigma <- rep(NA_real_, K)

    # constant
    V_Mn_asigma[1] <- a3 * Omega_Mn[i, i]
  ...
  # posterior mean: alpha_post = V_post * (V^-1 alpha + (Sigma^-1 kron X') y)
  V_post <- solve(solve(V_Mn) + kronecker(solve(Omega_Mn), t(X) %*% X))
  alpha_post <- V_post %*% (solve(V_Mn) %*% alpha_Mn + kronecker(solve(Omega_Mn), t(X)) %*% y)
...
# ---- SOC dummy A (Kilian-Lutkepohl style dummy) ----
estimate_soc_A <- function(df_train, vars, p, tau_socdum = 1, add_const = TRUE) {
  vm <- make_varmat_KK(df_train, vars = vars, p = p, add_const = add_const)
  Y <- vm$Y
  X <- vm$X
  ...
  # you used mu = colMeans(Y[(1:p),]) in your code (keep it)
  mu_socdum <- colMeans(Y[(1:p), , drop = FALSE])
  ...
  X_socdum <- rbind(
    ...
    t(rbind(
      ...
      kronecker(matrix(1, nrow = p, ncol = 1), diag(mu_socdum) / tau_socdum)
    ))
  )

  A_soc <- solve(t(X_socdum) %*% X_socdum, t(X_socdum) %*% Y_socdum) # K x M
  colnames(A_soc) <- vars
  A_soc
}

# ---- main: rolling/expanding 1-step forecast MSFE ----
msfe_1q_ahead <- function(df_var_long, vars, p,
  ...
  stopifnot(all(c("date", vars) %in% names(df)))

  Yall <- as.matrix(df[, vars, drop = FALSE])
  T_all <- nrow(Yall)
  M <- length(vars)
  K <- (if (add_const) 1L else 0L) + M * p

  # minimum training length so that (X'X) is invertible comfortably
  # effective obs = T_train - p must be > K
  min_T_train <- p + K + 1L
  if (T_all < min_T_train + 1L) stop("Not enough data for rolling 1-step forecasts.")

  # t indexes end of training sample; we forecast t+1
  t_start <- min_T_train
  t_end   <- T_all - 1L

  errs <- vector("list", length = t_end - t_start + 1L)
  kk <- 1L

  for (t in t_start:t_end) {
    df_train <- df[1:t, , drop = FALSE]
    Y_train  <- Yall[1:t, , drop = FALSE]

    # x_t built from last p actual observations up to time t
    x_t <- make_x_lastp(Y_train, p = p, add_const = add_const)  # length K

    # --- estimate A's on df_train ---
    ols <- estimate_ols_A_Omega(df_train, vars = vars, p = p, add_const = add_const)
    A_OLS <- ols$A
    A_MN  <- estimate_minnesota_A(df_train, vars = vars, p = p, a1 = a1, a2 = a2, a3 = a3, add_const = add_const)
    A_SOC <- estimate_soc_A(df_train, vars = vars, p = p, tau_socdum = tau_socdum, add_const = add_const)

    # --- 1-step point forecasts for y_{t+1} ---
    fc_ols <- as.numeric(x_t %*% A_OLS)
    fc_mn  <- as.numeric(x_t %*% A_MN)
    fc_soc <- as.numeric(x_t %*% A_SOC)

    y_next <- Yall[t + 1L, ]

    errs[[kk]] <- tibble::tibble(
      date = df$date[t + 1L],
      var  = vars,
      e_OLS = y_next - fc_ols,
      e_Minnesota = y_next - fc_mn,
      e_SOC = y_next - fc_soc
    )
    kk <- kk + 1L
  }

  df_err <- dplyr::bind_rows(errs)

  # MSFE by var & method
  msfe_tbl <- df_err |>
    dplyr::mutate(
      se_OLS = e_OLS^2,
      se_Minnesota = e_Minnesota^2,
      se_SOC = e_SOC^2
    ) |>
    dplyr::group_by(var) |>
    dplyr::summarise(
      MSFE_OLS = mean(se_OLS, na.rm = TRUE),
      MSFE_Minnesota = mean(se_Minnesota, na.rm = TRUE),
      MSFE_SOC = mean(se_SOC, na.rm = TRUE),
      .groups = "drop"
    )

  # (optional) overall average across vars (simple mean of MSFE across vars)
  msfe_overall <- tibble::tibble(
    MSFE_OLS = mean(msfe_tbl$MSFE_OLS, na.rm = TRUE),
    MSFE_Minnesota = mean(msfe_tbl$MSFE_Minnesota, na.rm = TRUE),
    MSFE_SOC = mean(msfe_tbl$MSFE_SOC, na.rm = TRUE)
  )

  list(msfe_by_var = msfe_tbl, msfe_overall = msfe_overall, errors = df_err)
}
...
res_msfe <- msfe_1q_ahead(
  df_var_long = df_var_long,
  vars = vars,
  p = p,
  start_date = as.Date("1995-01-01"),
  end_date   = as.Date("2019-12-31"),
...
res_msfe$msfe_by_var
res_msfe$msfe_overall
