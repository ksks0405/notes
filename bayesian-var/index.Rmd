...
- $\tau$が小さい（e.g. $10^{-4}$）→prior影響の影響大→**自己ラグの係数の和が1に近づく**
...
A_dobs_bigtau <- estimate_A_dobs(df_var, vars, p, tau = 10^4)
A_dobs_smalltau <- estimate_A_dobs(df_var, vars, p, tau = 10^(-4))
```
...
estimate_A_mn_post <- function(train_df, vars, p, a1=1, a2=0.5, a3=0.1) {
  ...
  # Minnesota prior mean (general M)
  alpha0 <- matrix(0, M*K, 1)
  idx_ownlag1 <- (0:(M-1))*K + (1 + (1:M))
  alpha0[idx_ownlag1] <- 1

  # Minnesota prior variance diag (general M)
  Vdiag <- rep(NA_real_, M*K)
  V_r <- c(1, rep((1:p)^(-2), each = M))

  for (i in 1:M) {
    tmp <- rep(NA_real_, K)
    tmp[1] <- (a3^2) * Omega_hat[i,i]  # const
    for (j in 1:M) {
      idx_j <- 1 + j + (0:(p-1))*M
  ...
  Vinv <- diag(1/Vdiag)
  Oinv <- solve((Omega_hat + t(Omega_hat))/2)
...
estimate_A_ols_old <- function(train_df, vars, p) {
  vm <- make_varmat_KK(train_df, vars = vars, p = p, add_const = TRUE)
  X <- vm$X; Y <- vm$Y
  solve(crossprod(X), crossprod(X, Y))
}

estimate_A_mn_post_old <- function(train_df, vars, p, a1=1, a2=0.5, a3=0.1) {
  # build X,Y,y
  vm <- make_varmat_KK(train_df, vars = vars, p = p, add_const = TRUE)
  X <- vm$X; Y <- vm$Y; y <- vm$y
  TT <- nrow(Y); M <- length(vars); K <- 1 + M*p
...
```{r REG msfe different tau}
msfe_table <- tibble()
for (tau_loop in c(1:20)/10){
  print(tau_loop)
  res_msfe <- msfe_1step_expanding(df_var_long, vars, p, soc_hyper = list(tau = tau_loop), dobs_hyper = list(tau_loop))
  msfe_table_loop <- mutate(res_msfe$msfe, tau = tau_loop)
  msfe_table <- bind_rows(msfe_table, msfe_table_loop)
}

msfe_table %>% 
  pivot_longer(cols = c(-var, -tau), names_to = "Model", values_to = "MSFE") %>% 
  mutate(Model = str_remove(Model, "MSFE_")) %>% 
  ggplot(mapping = aes(x = tau, y = MSFE, color = Model)) + 
  geom_line()+
  geom_point()+
...
```{r}
# --- packages: 既存に加えて zoo は already, quantmod already ---

# 四半期化ヘルパ：月次・日次でも四半期に落とす（デフォルトは四半期平均）
to_qtr_zoo <- function(x, agg = c("mean", "last")) {
  agg <- match.arg(agg)
  z <- zoo::zoo(as.numeric(x), zoo::as.yearmon(zoo::index(x)))  # month index
  # もし元が四半期や日次でも yearmon に落ちるので、年四半期へ集計できる
  if (agg == "mean") return(aggregate(z, zoo::as.yearqtr, mean, na.rm = TRUE))
  if (agg == "last") return(aggregate(z, zoo::as.yearqtr, function(v) tail(v, 1)))
}

# 7系列DL
getSymbols(c("GDPC1","GDPDEF","FEDFUNDS","PCECC96","GPDIC1","HOANBS","COMPRNFB"), src="FRED")

# 四半期系列（元が四半期でも「四半期へ強制」しておく）
rgdp_q  <- to_qtr_zoo(GDPC1,    agg = "mean")
def_q   <- to_qtr_zoo(GDPDEF,   agg = "mean")
ff_q    <- to_qtr_zoo(FEDFUNDS, agg = "mean")  # 金利は四半期平均が自然
c_q     <- to_qtr_zoo(PCECC96,  agg = "mean")
i_q     <- to_qtr_zoo(GPDIC1,   agg = "mean")
hours_q <- to_qtr_zoo(HOANBS,   agg = "mean")
comp_q  <- to_qtr_zoo(COMPRNFB, agg = "mean")

z_q <- na.omit(merge(rgdp_q, def_q, ff_q, c_q, i_q, hours_q, comp_q))
colnames(z_q) <- c("RGDP","DEF","FF","RC","RI","HOURS","RCOMP")

# 変換：あなたの既存方針に合わせる（log系は100*log、金利はlevel）
df_var7 <- tibble(
  date   = as.Date(as.yearqtr(index(z_q)), frac = 1),
  lRGDP  = 100*log(as.numeric(z_q[,"RGDP"])),
  lDEF   = 100*log(as.numeric(z_q[,"DEF"])),
  FF     = as.numeric(z_q[,"FF"]),
  lRC    = 100*log(as.numeric(z_q[,"RC"])),
  lRI    = 100*log(as.numeric(z_q[,"RI"])),
  lHOURS = 100*log(as.numeric(z_q[,"HOURS"])),
  lRCOMP = 100*log(as.numeric(z_q[,"RCOMP"]))
)

# 期間（既存を踏襲）
df_var_long <- df_var7 %>% filter(as.Date("1980-01-01") <= date, date <= as.Date("2019-12-31"))
df_var      <- df_var7 %>% filter(as.Date("1995-01-01") <= date, date <= as.Date("2019-12-31"))

vars7 <- c("lRGDP","lDEF","FF","lRC","lRI","lHOURS","lRCOMP")
...
var_1step_fc <- function(train_df, vars, p, A, add_const = TRUE) {
  ...
  hist <- Ymat[T_df:(T_df - p + 1), , drop = FALSE]  # rows: t, t-1, ..., t-p+1
  x_no_const <- as.vector(t(hist))                   # lag1(vars) -> lag2(vars) -> ...
  x <- if (add_const) c(1, x_no_const) else x_no_const
  drop(x %*% A)
}
...
msfe_1step_expanding <- function(df_long, vars, p,
  ...
  for (ii in seq_along(end_dates)) {
    ...
    if (nrow(test) != 1) next
    if (nrow(train) < (p + 10)) next

    A_ols  <- estimate_A_ols(train, vars, p)
    A_mn   <- do.call(estimate_A_mn_post, c(list(train_df=train, vars=vars, p=p), mn_hyper))
    A_soc  <- do.call(estimate_A_soc,     c(list(train_df=train, vars=vars, p=p), soc_hyper))
    A_dobs <- do.call(estimate_A_dobs,    c(list(train_df=train, vars=vars, p=p), dobs_hyper))
  ...
  df_msfe <- df_se %>%
    ...
    summarize(
      MSFE_OLS       = mean(se_ols,  na.rm = TRUE),
      MSFE_MN        = mean(se_mn,   na.rm = TRUE),
      MSFE_SOC       = mean(se_soc,  na.rm = TRUE),
      MSFE_DOBS      = mean(se_dobs, na.rm = TRUE),
      RMSE_OLS       = sqrt(MSFE_OLS),
      RMSE_MN        = sqrt(MSFE_MN),
      RMSE_SOC       = sqrt(MSFE_SOC),
      RMSE_DOBS      = sqrt(MSFE_DOBS),
      .groups="drop"
    )

  list(se = df_se, msfe = df_msfe)
}

# 実行（7変数）
p <- 5
res_msfe7 <- msfe_1step_expanding(df_var_long, vars = vars7, p = p, dobs_hyper = list(tau = 0.1), soc_hyper = list(tau = 0.1))
df_rmse7 <- res_msfe7$msfe
