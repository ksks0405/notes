...
- 誤差項は $\varepsilon_t \stackrel{i.i.d.}{\sim} N(0, \Sigma)$ と仮定する。

各時点の回帰変数を $x_t = (1, y_{t-1}', \ldots, y_{t-p}')'$
と定義する。
...
- 誤差項は $\varepsilon \sim N(0, \Sigma \otimes I_T)$に従う。
...
$\alpha = \mathrm{vec}(A)$ は $KM \times 1$ ベクトル。

Minnesota Prior では、係数ベクトル $\alpha$ に対して次の正規事前分布を仮定する：
...
すなわち、事前平均ベクトル $\underline{\alpha}_{Mn}$は基本的にゼロベクトルであるが、  
**各方程式における自分自身の第1ラグの係数のみ 1** に設定される。
...
tibble(OLS = alpha_OLS, `Minn. Prior` = alpha_Mn, Posterior = alpha_Mn_post) %>% round(2)
```
...
であり、事前分布
$$
\alpha \sim N(\underline{\alpha}_{Mn}, \underline{V}_{Mn})
$$
と次の対応関係を持つ：
...
このモデルは、**prior を dummy observation として加えた GLS 問題**である。

---

したがって、係数ベクトル $\alpha$ の **事後平均（MAP 推定量）**は、  
次の GLS 推定量として与えられる：
...
この表現は、Theil (1963) の mixed estimation に基づき、  
Kilian and Lütkepohl (2016, Section 5.2) において体系的に整理されている。
