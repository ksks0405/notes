...
msummary(var_est$varresult, stars = T, coef_map = coef_order, fmt = 3, estimate = "{estimate} ({std.error}){stars}", statistic = NULL,
...
A_Mn_unvec <- unvec(alpha_Mn, M = M, p = p, add_const = 1, vars = colnames(ts_var))
V_Mn_unvec <- unvec(diag(V_Mn), M = M, p = p, add_const = 1, vars = colnames(ts_var))
V_Mn_unvec_sorted <- V_Mn_unvec$A[coef_order, , drop = FALSE]
```
...
tibble(
  Equation = rep(colnames(A_Mn_unvec$A), each = K),
  Coef = rep(rownames(A_Mn_unvec$A), M),
...
# --- K と rownames の想定順序（あなたの X 構成と一致）---
K <- 1 + length(vars) * p
coef_names <- c("const", unlist(lapply(1:p, function(l) paste0(vars, ".l", l))))

# A_OLS（あなたのコードだと rownames が落ちてる可能性あり）
rownames(A_OLS) <- colnames(X)   # まずここで付けるのが自然
colnames(A_OLS) <- vars

# A_Mn_post（あなたのコードでは A_Mn_post は KxM に戻している）
rownames(A_Mn_post) <- rownames(A_OLS)
colnames(A_Mn_post) <- vars

# A_socdum（あなたのコードで計算した A_socdum）
rownames(A_socdum) <- colnames(X)  # X と同じ regressor 順序のはず
colnames(A_socdum) <- vars

# 念のため：順序チェック（欠けてたら止める）
stopifnot(identical(rownames(A_OLS), coef_names))
stopifnot(identical(rownames(A_Mn_post), coef_names))
stopifnot(identical(rownames(A_socdum), coef_names))
