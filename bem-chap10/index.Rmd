...
\begin{aligned}
\dot{\nu}&=\underbrace{\bar{\nu}}_{\underline{\nu}+N}+k,\\
\dot{\nu}\dot{s}^2 &= \underbrace{\bar{\nu}\bar{s}^2}_{ \underline{\nu}\underline{s}^{2}+\hat{\nu} \hat{s}^2+(\underline{\beta}-\bar{\beta})'\underline{Q}^{-1}(\underline{\beta}-\bar{\beta})  }+({\beta}-\bar{\beta})'\bar{Q}^{-1}({\beta}-\bar{\beta}).
\end{aligned}
$$
...
- Step 1（$β | h, y$ をドロー）：
  $β^{(s)} \sim N( \dot\beta , (h^{(s-1)})^{-1} \dot Q )$
  （直前の $h^{(s-1)}$ を固定して多変量正規分布からサンプル。）

- Step 2（$h | β, y$ をドロー）：
  $h^{(s)} \sim γ( \dot s^{-2} , \dot \nu)$
  ここで
    $\dot \nu = \bar ν + k$,
    $\dot \nu \dot s^2 = \bar{\nu} * \bar{s}^2 + (β^(s) - \bar{\beta})' * \bar{Q}^{-1} * (β^{(s)} - \bar{\beta})$.
  （Step 1 の $β^{(s)}$ を固定して Gamma 分布からサンプル。）

- 反復：
  上の Step 1–2 を S 回繰り返すと、{(β^(s), h^(s))} は p(β,h|y) のサンプル列になる
  （バーンインは適宜捨てる）。


[メモ] γ( s_dot^{-2} , ν_dot ) と Γ(shape, rate) の対応（R の rgamma は shape-rate）

BEM流に
  h | β, y ~ γ( s_dot^{-2} , ν_dot )
と書くとき、これは密度の核が
  p(h | β, y) ∝ h^{ν_dot/2 - 1} * exp( - (ν_dot * s_dot^2 / 2) * h ),   h>0
という意味。

したがって、R の rgamma(shape=, rate=)（＝ Γ(shape, rate)）で書くと
  h ~ Γ( shape = ν_dot/2 ,  rate = (ν_dot * s_dot^2)/2 ).

※ scale を使う実装では rate = 1/scale なので
  scale = 2 / (ν_dot * s_dot^2).




```{r REG Gibbs sampling, class.source='fold-hide'}
# Notation (matching the notes)
#   beta_bar = \bar{\beta}
#   Q_bar    = \bar{Q} = (\underline{Q}^{-1} + X'X)^{-1}
#   nu_bar   = \bar{\nu} = \underline{\nu} + N
#   s2_bar   = \bar{s}^2
#
# Conditional posteriors:
#   beta | h, y ~ N(beta_bar, h^{-1} Q_bar)
#   h | beta, y ~ Gamma(shape = (nu_dot)/2, rate = (nu_dot * s2_dot)/2)
# where
#   nu_dot      = nu_bar + k
#   nu_dot*s2_dot = nu_bar*s2_bar + (beta - beta_bar)' Q_bar^{-1} (beta - beta_bar)
...
k <- nrow(beta_bar)  # number of coefficients (dimension of beta)
...
for (s in seq_len(n_samples)) {
  ...
  # 1) Draw beta | h, y  ~ N(beta_bar, h^{-1} Q_bar)
  cov_beta <- solve(current_h * Qbar_inv)  # (h Q_bar^{-1})^{-1} = (h^{-1} Q_bar)
  drawn_beta <- MASS::mvrnorm(n = 1, mu = as.numeric(beta_bar), Sigma = cov_beta)
  ...
  #    nu_dot = nu_bar + k
  #    nu_dot*s2_dot = nu_bar*s2_bar + (beta-beta_bar)' Q_bar^{-1} (beta-beta_bar)
  #    => rate = (nu_dot*s2_dot)/2 = (nu_bar*s2_bar + quad)/2
  quad <- t(drawn_beta - beta_bar) %*% Qbar_inv %*% (drawn_beta - beta_bar)

  nu_dot <- nu_bar + k
  nu_dot_s2_dot <- nu_bar * s2_bar + as.numeric(quad[1, 1])
  s2_dot <- nu_dot_s2_dot / nu_dot

  shape_h <- nu_dot / 2
  rate_h  <- nu_dot_s2_dot / 2  # = (nu_dot*s2_dot)/2
...
# analytical mean for h under Gamma posterior: E[h] = 1 / s2_dot (same note as in ipynb)
analytical_mean_h <- if (!isTRUE(all.equal(s2_dot, 0))) 1.0 / s2_dot else NA_real_
