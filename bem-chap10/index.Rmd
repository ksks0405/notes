...
とすると、
$$
(y-X\beta)'(y-X\beta)=SSE+(\beta-\hat{\beta})'X'X(\beta-\hat{\beta})=\hat{\nu}\hat{s}^2+(\beta-\hat{\beta})'X'X(\beta-\hat{\beta})
$$
より、
$$
\boxed{\begin{alignedat}{2}
p(y|\beta, h)&= \left( \frac{h}{2\pi} \right)^{N/2} \exp \left[ -\frac{h}{2}\left\{ \underbrace{ \hat{\nu}\hat{s}^2 }_{=SSE}+(\beta-\hat{\beta})'X'X(\beta-\hat{\beta}) \right\} \right] &&\quad \text{(BEM 10.4)}, \\
\text{or}\\
...
p(y|\beta, h)&= \left( \frac{h}{2\pi} \right)^{N/2} \exp \left[ -\frac{h}{2}\left\{ \nu \hat{s}^2 +(\beta-\hat{\beta})'X'X(\beta-\hat{\beta}) \right\} \right] && \quad \dots \text{尤度}\\
p(h; \underline{s}, \underline{\nu}) &={C_\gamma}^{-1} h^{\underline{\nu}-2/2} \cdot \exp\left[-\frac{1}{2} h \frac{\underline{\nu}}{\underline{s}^{-2}}\right]  && \quad \dots \text{事前分布（Gamma部分）}\\
p(\beta|h,\underline{\beta}, \underline{Q}) &=\left( \frac{h}{2\pi} \right)^{k/2} |\underline{Q}|^{-\frac{1}{2}}\exp\left[ -\frac{h}{2}(\beta-\underline{\beta})'\underline{Q}^{-1}(\beta-\underline{\beta}) \right]  && \quad \dots \text{事前分布（正規分布部分）}
\end{alignedat}} $$
...
ベイズの定理より、
$$ \begin{aligned}
p(\beta, h |y) &\propto \underbrace{p(\beta, h; \underline{\beta},\underline{Q}, \underline{s}, \underline{\nu})}_{\text{ HPで規定されるparameterの事前分布}} \times \underbrace{ p(y|\beta, h) }_\text{parameterで条件づけられた尤度}\\
&= \quad p(\beta | h; \underline{\beta},\underline{Q})p(h; \underline{s}, \underline{\nu}) \quad \times \quad p(y|\beta, h)\\[1em]
\Rightarrow \,p(\beta, h |y) &\propto \left( \frac{h}{2\pi} \right)^{k/2} |\underline{Q}|^{-\frac{1}{2}}\exp\left[ -\frac{h}{2}(\beta-\underline{\beta})'\underline{Q}^{-1}(\beta-\underline{\beta}) \right]\quad\dots\text{事前分布（正規）}\\
&\quad \quad \times {C_\gamma}^{-1} h^{(\underline{\nu}-2)/2} \cdot \exp\left[-\frac{h}{2} \frac{\underline{\nu}}{\underline{s}^{-2}}\right] \quad\dots\text{事前分布（Gamma）}\\
&\quad \quad \times \left( \frac{h}{2\pi} \right)^{N/2} \exp \left[ -\frac{h}{2}\left\{ \hat{\nu} \hat{s}^2 +(\beta-\hat{\beta})'X'X(\beta-\hat{\beta}) \right\} \right] \quad\dots\text{尤度}\\
& \propto h^{\frac{k+\underline{\nu}-2+N}{2}} \exp \left[ -\frac{h}{2} \left\{\underline{\nu}\underline{s}^{2}+\hat{\nu} \hat{s}^2+(\beta-\underline{\beta})'\underline{Q}^{-1}(\beta-\underline{\beta})+(\beta-\hat{\beta})'X'X(\beta-\hat{\beta})\right\} \right].
\end{aligned} $$
...
$$
\begin{aligned}
 &(\beta-\underline{\beta})'\underline{Q}^{-1}(\beta-\underline{\beta})+(\beta-\hat{\beta})'X'X(\beta-\hat{\beta})\\
 =& (\beta-\bar{\beta})'\bar{Q}^{-1}(\beta-\bar{\beta})
...
=& (\beta-\bar{\beta})'\bar{Q}^{-1}(\beta-\bar{\beta})
+(\underline{\beta}-\hat{\beta})'[{\underline{Q}+(X'X)^{-1}}]^{-1}(\underline{\beta}-\hat{\beta})
\end{aligned}
$$
を用いると、
$$
...
-\frac{h}{2}\Big\{ &\underline{\nu}\underline{s}^{2}+ \hat{\nu}\hat{s}^2 +(\beta-\bar{\beta})'\bar{Q}^{-1}(\beta-\bar{\beta})\\ &
+(\underline{\beta}-\hat{\beta})'[{\underline{Q}+(X'X)^{-1}}]^{-1}(\underline{\beta}-\hat{\beta})
\Big\} \Bigg],\\
\text{or}\\
p(\beta,h\mid y) \propto h^{\frac{k+\bar{\nu}-2}{2}} \exp\Bigg[
-\frac{h}{2}\Big\{ &(\beta-\bar{\beta})'\bar{Q}^{-1}(\beta-\bar{\beta})+\bar{\nu} \bar{s}^2
\Big\} \Bigg].
\end{aligned}}
$$
$$
\boxed{\therefore \beta, h | y \sim NG(\bar{\beta}, \bar{Q}, \bar{s}^{-2}, \bar{\nu})}
$$
$$
\begin{alignedat}{2}
...
\bar{\nu}\bar{s}^2 &= \underline{\nu}\underline{s}^2+\hat{\nu}\hat{s}^2+(\underline{\beta}-\hat{\beta})'[{\underline{Q}+(X'X)^{-1}}]^{-1}(\underline{\beta}-\hat{\beta}), &&\quad \text{(Koop 3.13)}\\
\end{alignedat}
$$

## $\beta$の条件つき事後分布

$p(\beta,h|y)$に注目すると、$h$を定数項として考えた場合、$\beta$に依存するのは$\exp \{\,\}$内の第1項のみである。すなわち、
$$
...
$$
ここで再び$p(\beta,h|y)$に注目すると、$\beta$に依存するのは$\exp$内の第1項のみである。従って、
$$
\begin{aligned}
...
$$
\boxed{p(\beta,h\mid y) \propto h^{\frac{k+\bar{\nu}-2}{2}} \exp\Bigg[
-\frac{h}{2}\Big\{ (\beta-\bar{\beta})'\bar{Q}^{-1}(\beta-\bar{\beta})+\bar{\nu} \bar{s}^2
\Big\} \Bigg]\quad \text{（再掲）}}
$$

ここでは、条件付き事後分布のパラメータを$\{\dot{\beta},\dot{Q},\dot{\nu},\dot{s}\}$と書く。

$\beta$の条件付き事後分布は、解析解で導出済み。すなわち、

$$
\boxed{\beta|h \sim N(\dot{\beta},h^{-1}\dot Q),}
$$
$$
\text{where}\quad \begin{aligned}
\dot{Q} &= \bar{Q} = (\underline{Q}^{-1}+X'X)^{-1},\\
\dot{\beta} &= \bar{\beta}= \bar{Q}(\underline{Q}^{-1}\underline{\beta}+X'X\hat{\beta}).
\end{aligned}
$$
...
$$
\boxed{h|\beta \sim \gamma(\dot{s}^{-2},\dot{\nu}),}
$$

$$
\begin{aligned}
\dot{\nu}&=\underbrace{\bar{\nu}}_{\underline{\nu}+N}+k,\\
\dot{\nu}\dot{s}^2 &= \underbrace{\bar{\nu}\bar{s}^2}_{ \underline{\nu}\underline{s}^{2}+\hat{\nu} \hat{s}^2+(\underline{\beta}-\bar{\beta})'\underline{Q}^{-1}(\underline{\beta}-\bar{\beta})  }+({\beta}-\bar{\beta})'\dot{Q}^{-1}({\beta}-\bar{\beta}).
\end{aligned}
$$

$\dot{\nu}\dot{s}^2$は、$h$のmarginal posteriorのパラメータ $\bar{\nu} \bar{s}^2$ に$\beta$に依存する$({\beta}-\bar{\beta})'\bar{Q}^{-1}({\beta}-\bar{\beta})$を足したもの。

#### アルゴリズム

- 初期化：
  $h^{(0)} > 0$ を与える（例：$h^{(0)}=1$）。以後 $s=1,...,S$ を反復。

- Step 1（$\beta | h, y$ をドロー）：
  $\beta^{(s)} \sim N( \dot\beta , (h^{(s-1)})^{-1} \dot Q )$
  
  （直前の $h^{(s-1)}$ を固定して多変量正規分布からサンプル。）

- Step 2（$h | \beta, y$ をドロー）：

  $h^{(s)} \sim \gamma( \dot s^{-2} , \dot \nu)$, where $\dot \nu = \bar \nu + k$, $\dot \nu \dot s^2 = \bar{\nu} \bar{s}^2 + (\beta^{(s)} - \bar{\beta})'\dot{Q}^{-1}(\beta^{(s)} - \bar{\beta})$.
  
  （Step 1 の $\beta^{(s)}$ を固定して Gamma 分布からサンプル。）

- 反復：

  上の Step 1–2 を $S$ 回繰り返すと、$\{(\beta^{(s)}, h^{(s)})\}$は $p(\beta,h|y)$ のサンプル列になる
  （バーンインは適宜捨てる）。
  
上でみたとおり、それぞれの条件付き分布のうち、**可変のパラメータは**$\beta$**の**$h^{(s)}$**と**$h$**の**$\dot{s}$**（**$\beta^{(s)}$**を通じて変動）のみ。それ以外のパラメータ**（$\dot{\beta}, \, \dot{Q}, \, \dot{\nu}$）**はループを通じて不変**。
  
**メモ**: $\gamma(\dot{s}^{-2} , \dot{\nu})$ と $\Gamma(\text{shape}, \text{rate})$ の対応（R の `rgamma` は shape-rate）

BEM流に$h|\beta, y \sim \gamma( \dot s^{-2} , \dot \nu)$と書くとき、これは密度の核が

$$
p(h|\beta,y) \propto h ^{{\dot{\nu}}/2-1} \exp \left[-\frac{\dot{\nu}\dot{s}^2}{2}\right], \quad h>0
$$

したがって、Rの`rgamma`で書くと
$$
h \sim \Gamma\left(\text{shape}=\dot{\nu}/2, \,\text{rate}= \frac{\dot{\nu}\dot{s}^2}{2}\right)
$$

* scaleを使う実装では rate = 1/scaleなので、$\text{scale} = 2 / (\dot{\nu}\dot{s}^2)$。

#### 実装

下記はGibbs samplerを実装する関数。

```{r FUN Gibbs sampling refactor, class.source='fold-show'}
# Gibbs sampler as a function (Normal–Gamma regression posterior)
#   beta | h, y  ~ N(beta_bar, h^{-1} Q_bar)
#   h | beta, y  ~ Gamma(shape = (nu_bar + k)/2,
#                        rate  = (nu_bar*s2_bar + (beta-beta_bar)'Q_bar^{-1}(beta-beta_bar))/2)

gibbs_ng <- function(beta_bar, Q_bar, nu_bar, s2_bar,
                     h_init = 1.0, n_samples = 1000, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)

  k <- nrow(beta_bar)
  Qbar_inv <- solve(Q_bar)
  
  # variables to store the results
  beta1 <- numeric(n_samples)
  h_draw <- numeric(n_samples)
  ...
  for (s in seq_len(n_samples)) {
    ### Step 1: draw beta | h, y ###
    cov_beta <- Q_bar / current_h  # (h^{-1} Q_bar)
    b <- MASS::mvrnorm(n = 1, mu = as.numeric(beta_bar), Sigma = cov_beta)
    
    # update b -> store beta1 (2nd element)
    b <- matrix(b, ncol = 1)
    beta1[s] <- b[2, 1]

    ### Step 2: draw h | beta, y  ###
    quad <- as.numeric(t(b - beta_bar) %*% Qbar_inv %*% (b - beta_bar))  # scalar
    shape_h <- (nu_bar + k) / 2
    rate_h  <- (nu_bar * s2_bar + quad) / 2
...
$h^{(0)}=(1, 10000, 0.001)$の3パターンで実行。

ポイント：

- どの初期値から始めても早々に収束

- $h$が小さい（分散が大きい）→$\beta^{(1)}$の分散が大きくなり、収束に数回要している

- $h$が大きい（分散が小さい）→posterior mean（$\bar \beta$）にかなり近い$\beta^{(1)}$のを引くため、すぐに収束


```{r REG Gibbs sampling refactor, class.source='fold-show'}
h_inits <- c(1, 10000, 0.001)

trace_list <- lapply(h_inits, function(h0) {
  gibbs_ng(
    beta_bar = beta_post_analytical,
    Q_bar    = Q_post_analytical,
    nu_bar   = nu_post_analytical,
    s2_bar   = s2_post_analytical,
    h_init   = h0,
    n_samples = 1000
  )
})
...
trace_df$h_init <- factor(trace_df$h_init, levels = as.character(h_inits))
```

```{r EXP Gibbs sampling refactor}
# focus window (change if you want longer)
trace_df_100 <- subset(trace_df, iter <= 50)

# reference line for beta1 (analytical posterior mean)
beta1_ref <- beta_post_analytical[2, 1]

# Plot: beta1 trace (3 chains)
p_beta <- ggplot(trace_df_100, aes(x = iter, y = beta1, color = h_init)) +
  geom_line() +
  geom_hline(yintercept = beta1_ref, linetype = 2) +
  labs(
    title = expression("Gibbs trace: " * beta[1] * " (first 50 iters)"),
    x = "Iteration",
    y = expression(beta[1]),
    color = expression(h^{(0)})
  )

# Plot: h trace (3 chains)
p_h <- ggplot(trace_df_100, aes(x = iter, y = h, color = h_init)) +
  geom_line() +
  labs(
    title = "Gibbs trace: h (first 50 iters)",
    x = "Iteration",
    y = "h",
    color = expression(h^{(0)})
  )
