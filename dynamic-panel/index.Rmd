...
simulate_dynamic_panel <- function(
  ...
  # Individual fixed effects (mu_i)
  mu <- rnorm(N, 0, sigma_mu)
  
  # Idiosyncratic shocks (u_it)
  ...
  y[, 1] <- mu + u[, 1] / sqrt(pmax(1e-8, 1 - rho^2))
  
  for (tt in 2:T) {
    if (x_type == "none") {
      y[, tt] <- rho * y[, tt - 1] + mu + u[, tt]
    } else {
      y[, tt] <- rho * y[, tt - 1] + beta * x[, tt] + mu + u[, tt]
    }
  }
...
- 各idは同一の$\rho$のAR(1)に従う。

- id固有の切片のため、平均的な水準が異なる。

```{r IMP simulation data}
rho_true <- 0.8
data_small <- simulate_dynamic_panel(rho = rho_true, N = 10, T = 5, x_type = "none")
data_big <- simulate_dynamic_panel(rho = rho_true, N = 1000, T = 5, x_type = "none")
```
...
data_small %>% 
  ...
  labs(
    ...
    subtitle = expression(
      y[it] == rho * y[i,t-1] + mu[i] + u[it] ~ ",  " ~ rho == rho_true
    ),
    x = "Time (t)",
    y = expression(y[it]),
    color = "Individual (i)",
    caption = "Dashed lines: stationary means: mu_i / (1 - rho)"
  )
```
...
$$
y_{it} = \rho y_{i,t-1} + \mu_i + u_{it}.
$$

- $\mu_i$：個体固定効果（時間不変・観測不能）

- $u_{it}$：平均$0$のi.i.d.誤差項
...
固定効果$\mu_i$を除去するため、個体 $i$ごとの時間平均を引く：
...
$$
\tilde y_{it}
= \rho (y_{i,t-1} - \bar y_i) + (u_{it} - \bar u_i),
$$

すなわち

$$
\tilde y_{it} = \rho \tilde y_{i,t-1} + \tilde u_{it},
\qquad
...
## バイアスの源泉（Nickell bias）

(1) ラグ付き従属変数 $y_{i,t-1}$ は過去の誤差 $u_{i,t-1}$ を含む：

$$
y_{i,t-1} = \rho y_{i,t-2} + \mu_i + u_{i,t-1}.
$$

(2) within 後の誤差項 $\tilde u_{it}$ は平均誤差 $\bar u_i$ を含む。
$\bar u_i$ の中には $u_{i,t-1}$ が重み $1/T$ で含まれる。

したがって、within 変換後には機械的に

$$
\operatorname{Cov}(\tilde y_{i,t-1}, \tilde u_{it}) \neq 0
$$
...
$$
\operatorname{Bias}(\hat\rho_{FE}) \approx -\frac{1+\rho}{T-1}.
$$
...
通常のOLSで回帰。$\rho$が真の値（`r rho_true`）よりも小さくなる。
...
p_level <- ggplot(df_use2, aes(x = y_lag, y = u)) +
  ...
  labs(
    title = "y_{i,t-1} vs u_{it}",
    subtitle = "In the DGP, u_it is independent of past y",
...
In "uncollapsed form", the regression matrices are constructed as follows:
...
estimate_models <- function(df, iv_lags = 2:4) {
  ...
  coefs <- c(
    rho_pooled  = getc(pooled, "lag(y, 1)"),
    beta_pooled = getc(pooled, "x"),
    rho_fe      = getc(fe, "lag(y, 1)"),
    beta_fe     = getc(fe, "x"),
    rho_ab1     = getc(ab1, "lag(y, 1)"),
    beta_ab1    = getc(ab1, "x"),
    rho_ab2     = getc(ab2, "lag(y, 1)"),
    beta_ab2    = getc(ab2, "x")
  )
...
mc_run <- function(R = 200,
  ...
  colnames(coef_mat) <- c("rho_pooled","beta_pooled","rho_fe","beta_fe",
                          "rho_ab1","beta_ab1","rho_ab2","beta_ab2")
  
  diag_mat <- matrix(NA_real_, nrow = R, ncol = 4)
  colnames(diag_mat) <- c("ar2_p_ab1","ar2_p_ab2","hansen_p_ab1","hansen_p_ab2")
  
  for (r in seq_len(R)) {
    df  <- simulate_dynamic_panel(N=N, T=T, rho=rho, beta=beta,
  ...
  summary_tbl <- rbind(
    pooled_rho  = summarize_est(coef_df$rho_pooled,  rho),
    fe_rho      = summarize_est(coef_df$rho_fe,      rho),
    ab1_rho     = summarize_est(coef_df$rho_ab1,     rho),
    ab2_rho     = summarize_est(coef_df$rho_ab2,     rho),
...
# ---- Run (exogenous x) ----
ans_exog <- mc_run(R=200, N=5000, T=8, rho=0.7, beta=1.0, iv_lags=2:4, x_type="exogenous")
